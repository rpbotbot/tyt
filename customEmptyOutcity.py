...
class PyEmptyOutcity:

    def __init__(self):
        print('starting empty outcity node')

        self.service_start = rospy.Service('/external_process/trigger_detecting8', StringTrigger, self.start_handler)
        self.service_stop = rospy.Service('/external_process/stop_detecting8', StringTrigger, self.stop_handler)
    ...
    def handle_topik(self, data):
        ...
        if (data.data == "outcityemptybox=clear"):
            self.flag = False
    ...
    def start_handler(self, req):
        ...
        while(self.flag):
            print('waiting topic outcityemptybox to be clear')
            rospy.sleep(1)
