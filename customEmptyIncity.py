...
class PyEmptyIncity:
    ...
    def __init__(self):
        print('starting empty incity node')

        self.service_start = rospy.Service('/external_process/trigger_detecting7', StringTrigger, self.start_handler)
        self.service_stop = rospy.Service('/external_process/stop_detecting7', StringTrigger, self.stop_handler)
    ...
    def handle_topik(self, data):
        ...
        if (data.data == "incityemptybox=clear"):
            self.flag = False
    ...
    def start_handler(self, req):
        ...
        while(self.flag):
            print('waiting topic incityemptybox to be clear')
            rospy.sleep(1)
